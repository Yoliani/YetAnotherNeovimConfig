...
cmd([[augroup custom_statusline]])
cmd([[autocmd!]])
cmd([[autocmd VimEnter,ColorScheme * call v:lua.tab.statusline.set_colors()]])
cmd([[augroup END]])
vim.o.statusline = '%!v:lua.tab.statusline.setup()'
...
local function mode_highlight(mode)
  ...
  elseif vim.tbl_contains({'v', 'V', ''}, mode) then
    cmd [[hi StMode guibg=#fe8019 guifg=#3c3836]]
    vim.cmd [[hi StModeSep guifg=#fe8019]]
  elseif mode == 'R' then
    cmd [[hi StMode guibg=#8ec07c guifg=#3c3836]]
    cmd [[hi StModeSep guifg=#8ec07c]]
  else
    cmd('hi StMode guibg='..c.normal_fg..' guifg='..c.normal_bg..' gui=NONE')
    cmd('hi StModeSep guifg='..c.normal_fg..' guibg='..c.statusline_bg..' gui=NONE')
  end
end
